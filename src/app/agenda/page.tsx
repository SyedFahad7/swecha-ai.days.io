                ...
                {day.events.map((event) => (
                  ...
                  >
                    {event.type === 'break' ? (
                      ...
                      <div className={`relative overflow-hidden rounded-lg border bg-gradient-to-br ${getSessionTypeStyles(event.type)}`}>
                      ...
                      <Link href={`/agenda/${event.id}`} className="block">
                        <div className={`relative overflow-hidden rounded-lg border bg-gradient-to-br ${getSessionTypeStyles(event.type)} group-hover:border-yellow-500/50 transition-colors`}>
                          <div className="relative p-6">
                            {/* Time and Track */}
                            <div className="flex items-center gap-4 mb-4">
                              <div className="text-yellow-300 font-mono">{event.time}</div>
                            ...
                            <div className="absolute top-6 right-6">
                              <div className={`px-3 py-1 rounded-full text-xs uppercase tracking-wider border ${
                                event.type === 'keynote' ? 'border-purple-500/30 text-purple-300' :
                                event.type === 'panel' ? 'border-cyan-500/30 text-cyan-300' :
                                event.type === 'workshop' ? 'border-green-500/30 text-green-300' :
                                event.type === 'talk' ? 'border-yellow-500/30 text-yellow-300' :
                                event.type === 'entertainment' ? 'border-pink-500/30 text-pink-300' :
                                event.type === 'opening' ? 'border-indigo-500/30 text-indigo-300' :
